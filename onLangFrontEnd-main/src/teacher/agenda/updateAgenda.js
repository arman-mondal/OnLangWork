...
export default class UpdateAgenda extends React.Component{
  ...
  componentDidMount () {
    ...
    this.getSlots(savedToken);
    this.getSelectedSlots(savedToken);
  ...
  getSelectedSlots(savedToken){
     axios({
      method: "get",
      url:  configData.SERVER_URL + 'slots/getTeacherSlot',
      headers: { 
        "Content-Type": "application/x-www-form-urlencoded",
        "authtoken" : savedToken
      },
        ...
        console.log(resp.data)
        if(resp.data.code === 200){
          var myPreSelectedSlots = [];
          resp.data.selectedSlots.forEach((selectedSlot)=>{
              var day  = this.state.days.find(function(preDay) { 
                return preDay.id == selectedSlot.dayid
              })
              var mySlot  = this.state.slots.find(function(slot) { 
                return slot.slotid == selectedSlot.slotid
              })
              myPreSelectedSlots.push( { slot : mySlot, day: day});
          })

          console.log(myPreSelectedSlots);
          this.setState({
            selectedDays : resp.data.selecteddays,
            selectedSlots : myPreSelectedSlots,
            days : this.state.days.filter(day => !resp.data.selecteddays.find(selectedDay => selectedDay.id == day.id))
          });
        }else{
          swal({
            title: "Server Not Responding",
  ...
  hitAgendaUpdatenApi = (e) => {
    ...
    axios({
        method: "post",
        url: configData.SERVER_URL + 'teachers/updateagenda',
        data: bodyFormData,
        headers: { 
          "Content-Type": "application/x-www-form-urlencoded",
          "authtoken" : savedToken
              ...
              <div className="col-lg-12 col-md-12 mt-6 mt-lg-0">
                <div className="box price featured no-padding">
                  <Card className="p-3">
                    <h3>Update Agenda</h3>
                    <div className="row d-flex align-items-center justify-content-center div-scroll-y">
                      <h5 className="text-center">Please select your avaliable slots for classes</h5>
                      {this.state.selectedDays.map((selectedDay) => ( 
                            ...
                            <ul className="list-group selectbox">
                              ...
                              { this.state.slots.map((slot) => (
                                <li className="list-group-item list-group-item-secondary" onClick={e => this.addSlot(slot,selectedDay)}> 
                                  {slot.name} ({moment(slot.starttime).format('h:mm A')}-{moment(slot.endtime).add(1, "minute").format('h:mm A')})
                                </li>
                                )
                              )}
                            </ul>
                          ...
                          <div className="col-lg-5 col-md-5">
                            ...
                            <ul className="list-group selectbox">
                              {this.state.selectedSlots.filter(selectedSlot => selectedSlot.day.id == selectedDay.id).map(filteredSelectedSlot => (
                                ...
                                <div className="selectInnerDiv">
                                    <div className="studentInfo"> 
                                      {filteredSelectedSlot.slot.name} {moment(filteredSelectedSlot.slot.starttime).format('h:mm A')}-{moment(filteredSelectedSlot.slot.endtime).add(1, "minute").format('h:mm A')})
                                    </div>
                                    <div className="badgeInfo">
                ...
                <div className="box price featured no-padding">
                  <Card className="p-3">
                    ...
                    <div className="btn-wrap-next">
                      ...
                      <button type="submit" className="btn-buy button-Next" onClick={this.hitAgendaUpdatenApi}>Confirm</button>
                    </div>
                  </Card>
                </div>
              </div>
