...
router.post("/getTeacherClasses", urlencodedParser, async(req, res) => {
    const myClass = await prisma.$queryRaw`SELECT * FROM class INNER JOIN course ON course.courseid = class.courseid INNER JOIN classtiming ON classtiming.classid = class.classid INNER JOIN slots ON slots.slotid = classtiming.slotid INNER JOIN packages ON packages.packageid = class.packageid WHERE class.teacherid = ${req.body.teacherid} AND classtiming.dayid = ${req.body.dayid}`
	if(myClass.length > 0){
		res.json({
			"code": 200,
...
router.post("/getStudentTimetable", urlencodedParser, async(req, res) => {
	const myClass = await prisma.$queryRaw`SELECT DISTINCT(class.classid), class.*, course.coursename, slots.starttime, slots.endtime, classtiming.dayid FROM class INNER JOIN course ON course.courseid = class.courseid INNER JOIN classstudents ON classstudents.classid = class.classid INNER JOIN classtiming ON classtiming.classid = class.classid INNER JOIN slots ON slots.slotid = classtiming.slotid WHERE classstudents.studentid = ${req.body.studentid}`
	if(myClass.length > 0){
		const days = await prisma.days.findMany()
		res.json({
			"code": 200,
...
router.get("/getclassforcourse/:courseid", urlencodedParser, async(req, res) => {
...
router.get("/getliveclassforcourse/:classid", urlencodedParser, async(req, res) => {
	const teacher = await prisma.teacher.findMany({ where : { jwt : req.headers.authtoken }})
	if(teacher.length > 0){
...
router.post('/upload-file',upload.any(), async (req, res) => {
	var uuid = req.body.uuid;
	const dropbox = new Dropbox({ accessToken: dropboxToken, fetch });
    ...
    const filecreate = await prisma.liveclassfiles.create({
		data: {
			filename: sharedLink,
			type: req.body.type,
